...
configtx.yaml是组织、通道配置文件，用于定义组织和通道信息，并基于此文件生成排序系统创世块和通道创世块。

官方提供的示例配置文件在"path/to/fabric/sampleconfig/configtx.yaml"，示例配置文件内容如下，文件比较长，其中很详细的注释了每个配置项的功能：
```
                    ...
                    - <<: *SampleOrg
                      Policies:
                          ...
                          Admins:
                              ...
                              Rule: "OR('SampleOrg.member')"
```
示例配置文件中，在Profiles段中，提供了多种不同类型场景下通道的配置示例，这个需要仔细看注释内容，参考合适的例子编写自己的配置文件。
...
Organizations:
    ...
    - &Orderer
        Name: OrdererMSP
        ID: OrdererMSP
        MSPDir: crypto-config/ordererOrganizations/cardchain.com/msp
        Policies: &SampleOrgPolicies
            Readers:
                Type: Signature
                Rule: "OR('OrdererMSP.member', 'OrdererMSP.admin', 'OrdererMSP.peer', 'OrdererMSP.client')" # "OR('OrdererMSP.admin', 'OrdererMSP.peer', 'OrdererMSP.client')"
            Writers:
                Type: Signature
                Rule: "OR('OrdererMSP.member', 'OrdererMSP.admin', 'OrdererMSP.peer', 'OrdererMSP.client')" # "OR('OrdererMSP.admin', 'OrdererMSP.peer', 'OrdererMSP.client')"
            Admins:
    ...
    - &OrgA
        ...
        MSPDir: crypto-config/peerOrganizations/orga.cardchain.com/msp

        Policies: &SampleOrgPolicies
            Readers:
                Type: Signature
                Rule: "OR('OrgAMSP.admin', 'OrgAMSP.peer', 'OrgAMSP.client', 'OrgA.member')"
            Writers:
                Type: Signature
                Rule: "OR('OrgAMSP.admin', 'OrgAMSP.peer', 'OrgAMSP.client', 'OrgA.member')" # "OR('OrgA.member')"
            Admins:
                Type: Signature
    ...
    - &OrgB
        ...
        MSPDir: crypto-config/peerOrganizations/orgb.cardchain.com/msp

        Policies: &SampleOrgPolicies
            Readers:
                Type: Signature
                Rule: "OR('OrgBMSP.admin', 'OrgBMSP.peer', 'OrgBMSP.client', 'OrgB.member')"
            Writers:
                Type: Signature
                Rule: "OR('OrgBMSP.admin', 'OrgBMSP.peer', 'OrgBMSP.client', 'OrgB.member')" # "OR('OrgB.member')"
            Admins:
                Type: Signature
...
Application: &ApplicationDefaults
    ACLs: &ACLsDefault
        ...
        peer/ChaincodeToChaincode: /Channel/Application/Readers
        event/Block: /Channel/Application/Readers
        event/FilteredBlok: /Channel/Application/Readers
...
```
文件中，定义了Orderer，OrgA，OrgB三个组织，给出了Orderer和Channel的默认配置，最后，在Profiles段中给出了OrdererOrg和TxChannel的配置。OrdererOrg部分用于生成排序系统的创世块，TxChannel部分用于生成“TxChannel”通道的创世块。
