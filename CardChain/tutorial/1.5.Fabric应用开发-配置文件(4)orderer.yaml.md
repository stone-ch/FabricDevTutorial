...
orderer.yaml是orderer节点的配置文件，凡是orderer需要设置的内容，全在这里找。

官方提供的示例配置文件在"path/to/fabric/sampleconfig/orderer.yaml"，示例配置文件内容如下，文件短了很多，其中也很详细的注释了每个配置项的功能：
```
...
配置文件中需要注意的部分是：
1. General->ListenAddress，orderer的监听地址
2. General->ListenPort，orderer的监听端口
3. General->TLS，通讯证书相关内容
4. ** General->GenesisFile，排序通道创世块文件名，生成的排序创世块必须和此处一致**
5. General->LocalMSPDir，msp相关文件路径
6. ** General->LocalMSPID，此处ID必须和configtx.yaml文件中Orderer组织的ID一致**
...
General:
    ...
    TLS:
        Enabled: false
        PrivateKey: crypto-config/ordererOrganizations/cardchain.com/orderers/orderer.cardchain.com/tls/server.key
        Certificate: crypto-config/ordererOrganizations/cardchain.com/orderers/orderer.cardchain.com/tls/server.crt
        RootCAs:
          - crypto-config/ordererOrganizations/cardchain.com/orderers/orderer.cardchain.com/tls/ca.crt
        ClientAuthRequired: false
        ClientRootCAs:
    ...
    LocalMSPDir: crypto-config/ordererOrganizations/cardchain.com/orderers/orderer.cardchain.com/msp
...
FileLedger:
    Location: orderer/production/orderer
    Prefix: hyperledger-fabric-ordererledger
...
同样，General->TLS下各证书也是根据crypto-config.yaml文件使用cryptogen工具生成的，具体路径需要根据实际情况设置。
