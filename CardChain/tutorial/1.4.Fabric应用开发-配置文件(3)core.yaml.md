...
core.yaml是peer的配置文件，需要配置的内容比较多，凡是peer需要设置的内容，全在这里找就对了。

官方提供的示例配置文件在"path/to/fabric/sampleconfig/core.yaml"，示例配置文件内容如下，文件更长，其中也很详细的注释了每个配置项的功能：
```
...
配置文件中需要注意的部分是：
1. peer->address，peer的IP地址；
2. peer->gossip->tls，该部分的通讯证书路径很关键，如果路径不正确将无法和peer进行通讯；
3. chaincode->node，确定chaincode的运行模式，有"dev"和"net"两种。"dev"模式用于开发调试chaincode时使用，"net"模式用于正常运行网络。
4.ledger->state->stateDatabase，设置保存状态（state）的数据库。有“goleveldb”和“CouchDB”两种数据库可选。其中“CouchDB”支持富查询，功能较“goleveldb”更丰富。
...
peer:
    ...
    tls:
        ...
        key:
            file: crypto-config/peerOrganizations/orga.cardchain.com/peers/peer0.orga.cardchain.com/tls/server.key
        rootcert:
            file: crypto-config/peerOrganizations/orga.cardchain.com/peers/peer0.orga.cardchain.com/tls/ca.crt
        clientRootCAs:
            files:
              - crypto-config/peerOrganizations/orga.cardchain.com/peers/peer0.orga.cardchain.com/tls/ca.crt
        clientKey:
            file:
        clientCert:
            file:
    ...
    mspConfigPath: crypto-config/peerOrganizations/orga.cardchain.com/peers/peer0.orga.cardchain.com/msp
...
其中，peer->gossip->tls下各证书都是根据crypto-config.yaml文件使用cryptogen工具生成的，具体路径需要根据实际情况设置。
