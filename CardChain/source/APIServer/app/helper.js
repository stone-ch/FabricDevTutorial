...
var ORDERERS = hfc.getConfigSetting('orderers');
...
for (let key in ORGS) {
	...
	cryptoSuite.setCryptoKeyStore(hfc.newCryptoKeyStore({path: getKeyStoreForOrg(ORGS[key].mspid)}));
	client.setCryptoSuite(cryptoSuite);

	let channel = client.newChannel('cardchainchannel');
	//channel.addOrderer(newOrderer(client));
	setupOrderers(channel, client);

	clients[key] = client;
	channels[key] = channel;

	setupPeers(channel, client);
...
function setupPeers(channel, client) {
	for (let key in PEERS) {
		let data = fs.readFileSync(path.join(__dirname, PEERS[key]['tls_cacerts']));
		let peer = client.newPeer(
			PEERS[key].url,
			{
				pem: Buffer.from(data).toString(),
				'ssl-target-name-override': PEERS[key]['grpcOptions'].ssl-target-name-override,
...
function setupOrderers(client) {
	for (let key in ORDERERS) {
		let data = fs.readFileSync(path.join(__dirname, ORDERERS[key]['tls_cacerts']));
		let orderer = client.newOrderer(
			ORDERERS[key].url,
			{
				pem: Buffer.from(data).toString(),
				'ssl-target-name-override': ORDERERS[key]['grpcOptions'].ssl-target-name-override,
	            'request-timeout' : 450000
			}
		);
		orderer.setName(key);
